...
const API = {
    ...
    register: (name, email, pass, success) => {
        axios.post(`${host}/api/users`, { name: name, email: email, password: pass }).then(res => {
            success(res);
        }).catch(err => {
            success(err);
        })
        ...
        axios.get(`${host}/api/Posts`, {
            params: {
                filter: {
                    skip: skip,
                    limit: 5,
                    include: 'PostImage',       // this includes the PostImage since we created a relation between Post and PostImage
                    fields: {
                        id: true,
                        title: true,
                        slug: true,
    ...
    getPostBySlug: (slug, token, success) => {
        axios.get(`${host}/api/Posts/findOne?access_token=${token}`, {
            params: {
                filter: {
                    where: { slug: slug },
                    include: { Comments: 'Profile' }     // include related model Comments and the Profile
                }
            }
        }).then(res => {
            success(res);
